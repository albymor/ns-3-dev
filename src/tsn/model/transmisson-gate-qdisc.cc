  ...
  if(m_gateState.state)
    {
      if(m_gateState.interval.IsStrictlyPositive())
        {
          Time transmissionDuration = MicroSeconds(100); // (paketref);
          Time window = m_gateState.interval - GetDeviceTime();

          //std::cout << "transmissionDuration " << transmissionDuration << " window " << window.GetMicroSeconds() << " " << GetDeviceTime() << " " << Simulator::Now ().GetMilliSeconds() << " " << m_gateState.interval.GetMilliSeconds() << std::endl;
