...
TasQueueDisc::DoDequeue (void)
{
  ...
  Time now = GetDeviceTime();
   ...
   if(m_transmitting.IsExpired())
    {
       if(m_channel != 0 && !m_channel->IsStateIdle()){
            
            m_transmitting = Simulator::Schedule(m_channel->GetDelayUntilIdle(),&TasQueueDisc::Run,this);
            //std::cout << this << " Channel is busy " << now.GetNanoSeconds() << " waiting until " << m_channel->GetDelayUntilIdle().GetNanoSeconds() << " because " << m_channel->GetState() << std::endl;
            return 0;
       }
          ...
          if(returnVal != 0)
            {
              ...
              //std::cout << this << " Paket Dequeued " << now.GetNanoSeconds() << std::endl;
              return returnVal;
            }
        }
    }
     ...
     if(m_gateListConfig.at(index).GetGateState() != gateState || m_gateListConfig.at(index).GetNextEvent() != nextEvent)
       {
         ...
         //std::cout << this << " Queue: " << index <<" NewState: " << newState.state << " Interval: " << newState.interval.GetMicroSeconds() << " Sim time : " << Simulator::Now ().GetNanoSeconds() << std::endl;
