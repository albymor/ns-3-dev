...
#ifndef SAFETY_UDP_ECHO_CLIENT_H
#define SAFETY_UDP_ECHO_CLIENT_H
 ...
 */
class SAFETYUdpEchoClient : public Application 
{
public:
  /**
  ...
  void SetFailSafeManager (Ptr<FailSafeManager> fsm);
...
private:
  ...
  void EnableFailSafe (std::string Reason);
  ...
  void ScheduleTransmitNow ();

  void ScheduleWdtExpired ();
  void DestroyWdtExpired ();
  ...
  Time m_rtt;
  Ptr<Packet> aCopy;
  Ptr<FailSafeManager> m_failsafeManager;
  uint8_t m_isFailsafeSource;
  Time m_FailSafeTriggerTime;
  Time m_WdtPeriod;
  EventId m_WdtExpireEvent;
  uint32_t m_sequnceNumber;
  bool m_isPending;
  bool m_wdtenabled;
